...
public class XPRNSDKController {
    ...
    @Value("${xprnetwork.endpoints}") private String[] endpoints;
    ...
    public String testTransaction() throws Exception {
        ...
        int endpointIndex = 0;
        int successEndpoint = 0;

        while (successEndpoint != 200) {
            endpointIndex = (int) (Math.random() * (endpoints.length - 1));
            URL url = new URL(endpoints[endpointIndex]+"/v1/chain/get_info"); // 호출할 외부 API 를 입력한다.
            HttpURLConnection conn = (HttpURLConnection) url.openConnection();
            conn.setRequestMethod("GET");
            conn.setRequestProperty("Content-type", "application/json; charset=UTF-8");
            successEndpoint = conn.getResponseCode();
        }


        IRPCProvider rpcProvider = new EosioJavaRpcProviderImpl(endpoints[endpointIndex]);
